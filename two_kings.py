...
This module implements the Two Kings game, a simplified version of chess played 
on a 5x5 board between only two kings. The rules of the game are as follows:

The kings start on opposite sides of the board and they can only move 
horizontally and vertically (not diagonally). White moves first, and the goal 
of the game is to take the opponents king. If no one succeeds to do so within 
10 moves, the game is declared a draw.

Because diagonal moves are not allowed, the game is such that it is impossible 
for white to win, and optimal play results in a black win in 5 moves.
...
Running this file as a script calls play(), allowing the user to play Two Kings 
