...
def action_mask(state: torch.Tensor) -> torch.BoolTensor:
    ...
    :param state: bx4x2x2 tensor. Each batch b has 4 2x2 planes: P1 pos, P2 pos, color, move number
    :return mask: bx4x2x2 tensor. Each batch b has 4 2x2 planes: move up, down, left, right
    """
    # TODO: Figure out a more efficient / "torchy" implementation

    num_batches = state.size(0)
    mask = torch.zeros(num_batches, 4, 2, 2, dtype=torch.bool)
...
class EnvTwoKings2x2:
    """Environment for Two Kings 2x2 game."""
    def __init__(self, state=None):
        ...
        if isinstance(state, torch.Tensor):
            ...
            self.state = torch.zeros(1, self.in_features, self.board_size, 
                                     self.board_size)
            self.state[0, 0, 1, 0] = 1 # P1 at a1
            self.state[0, 1, 0, 1] = 1 # P2 at b2
...
def play(net: Network, print_move=True):
    ...
    def selection_screen():
        ...
        title = font_large.render("TWO KINGS 2x2", True, WHITE)
