...
def action_mask(state: torch.Tensor) -> torch.BoolTensor:
    ...
    :return mask: Each batch has 4 3x3 planes: move up, down, left, right
    """
...
class EnvTwoKings3x3:
    """Environment for Two Kings 3x3 game."""
    def __init__(self, state=None):
        ...
        if isinstance(state, torch.Tensor):
            ...
            self.state = torch.zeros(1, self.in_features, self.board_size, 
                                     self.board_size)
            self.state[0, 0, 2, 1] = 1 # P1 at b1
            self.state[0, 1, 0, 1] = 1 # P2 at b3
